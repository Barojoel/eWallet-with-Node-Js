    ...
    Transactions.aggregate([
        ...
        Transactions.find({$or: [{recipient: req.user._id}, {sender: req.user._id}]}).populate('recipient').populate('sender').sort({createdAt: 'desc'}).limit(5).exec((err, userTransactions) => {
            Transactions.aggregate([
                ...
                const months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December '];                const start = date.getMonth() >= 6 ? date.getMonth()-5 : 7+date.getMonth();
                let values = [];
                let colors = [];
...
//Handling Transaction APIs
//I have deleted and added to the routes

router.get('/wallet', auth.checkAuth, (req, res) => {
    res.render('app', {user: req.user, page: 'wallet'});
});

router.get('/addincomexp', auth.checkAuth, (req, res) => {
    res.render('app', {user: req.user, page: 'addincomexp'});
});
...
router.post('/addincomexp',  (req, res) => {
    res.render('app', {user: req.user, page: 'addincomexp'});
});
        ...
        Coupons.findOne({code: req.body.code}, (err, coupon) => {
            ...
            if(coupon) {
                if(coupon.useDate == undefined) {
                    ...
                    transaction.save();
                    res.render('app', {user: req.user, page: 'coupon', message: `Coupon redeemed ${coupon.code}. Top-up amount: ${coupon.amount} zlotys`});
                } else {
                    res.render('app', {user: req.user, page: 'coupon', message: `This coupon has already been used`});
                }
                
            } else {
                res.render('app', {user: req.user, page: 'coupon', message: 'Invalid Coupon'});
            }
        });
    } else {
        res.render('app', {user: req.user, page: 'coupon', message: 'Enter Coupon'});
    }
});

router.post('/addcard', auth.checkAuth, (req, res) => {
    if(!/^(?:[0-9]{4}-){3}[0-9]{4}$/.test(req.body.card)) {
        res.render('app', {user: req.user, page: 'addcard', message: 'Enter a valid credit card number', values: req.body});
    } else if(!/^0[1-9]|1[0-2]\/[0-9]{2}$/.test(req.body.expirationDate)) {
        res.render('app', {user: req.user, page: 'addcard', message: 'Enter a valid expiration date', values: req.body});
    } else if(!/^[0-9]{3}$/.test(req.body.cvv)) {
        res.render('app', {user: req.user, page: 'addcard', message: 'Enter a valid CVV code', values: req.body});
    } else {
        // TODO: Add card to database
        res.render('app', {user: req.user, page: 'addcard', message: 'Unable to link card to account'});
    }
});
            ...
            if(user) {
                if(req.body.email != req.user.email) {
                    if(amount > 0 && amount <= req.user.balance) {
                        ...
                        res.render('app', {user: req.user, page: 'payments', message: 'You dont have enough funds in your account', transactions: JSON.stringify(userTransactions)});
                    }
                } else {
                    res.render('app', {user: req.user, page: 'payments', message: 'The given email is assigned to your account', transactions: JSON.stringify(userTransactions)});
                }
            } else {
                res.render('app', {user: req.user, page: 'payments', message: 'User with this email address does not exist', transactions: JSON.stringify(userTransactions)});
            }
        });
        } else {
            res.render('app', {user: req.user, page: 'payments', message: 'Fill in required fields', transactions: JSON.stringify(userTransactions)});
        }
    });
});
